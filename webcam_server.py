...
last_count_time = 0
COOLDOWN_TIME = 0.8  # in seconds

def checkJumpCrouch(image, results, MID_Y=250, draw=False, threshold=50):
    global jump_count, crouch_count, calories_burnt, previous_posture, last_count_time
    height, width, _ = image.shape
    output_image = image.copy()

    if not results.pose_landmarks:
        return image, 'No Detection'

    left_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER].y * height)
    right_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_SHOULDER].y * height)
    actual_mid_y = abs(right_y + left_y) // 2

    lower_bound = MID_Y - threshold
    upper_bound = MID_Y + threshold
    ...
    if previous_posture == 'Standing':
        if current_posture == 'Jumping' and current_time - last_count_time > COOLDOWN_TIME:
            jump_count += 1
            calories_burnt += CALORIES_PER_JUMP
            last_count_time = current_time
        elif current_posture == 'Crouching' and current_time - last_count_time > COOLDOWN_TIME:
            crouch_count += 1
